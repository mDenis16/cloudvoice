...
export let friendRespondAction = authUserMiddlewareAction(EAccessLevel.USER, async (requesterUser: UserJWTModel,request: ActionFriendRespondRequest) : Promise<ActionFriendRespondResponse> => {
  "use server";
  try {
    ...
    console.info(`[DEBUG] ${requesterUser.id} is now friend with ${invite.receiverId}!`);
